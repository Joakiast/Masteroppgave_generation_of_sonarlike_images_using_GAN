...
def Generator():
  ...
  down_stack = [
    downsample(128, 2, apply_batchnorm=False),  # (batch_size, 128, 128, 64)
    downsample(256, 2),  # (batch_size, 64, 64, 128)
    downsample(512, 2),  # (batch_size, 32, 32, 256)
    downsample(1024, 2),  # (batch_size, 16, 16, 512)
    downsample(1024, 2),  # (batch_size, 8, 8, 512)
    downsample(1024, 2),  # (batch_size, 4, 4, 512)
    downsample(1024, 2),  # (batch_size, 2, 2, 512)
    downsample(1024, 2),  # (batch_size, 1, 1, 512)
  ]

  up_stack = [
    upsample(1024, 2, apply_dropout=True),  # (batch_size, 2, 2, 1024)
    upsample(1024, 2, apply_dropout=True),  # (batch_size, 4, 4, 1024)
    upsample(1024, 2, apply_dropout=True),  # (batch_size, 8, 8, 1024)
    upsample(1024, 2),  # (batch_size, 16, 16, 1024)
    upsample(512, 2),  # (batch_size, 32, 32, 512)
    upsample(256, 2),  # (batch_size, 64, 64, 256)
    upsample(128, 2),  # (batch_size, 128, 128, 128)
  ]
...
LAMBDA = 200 # jo større lambda er jo høyere skal likheten mellom treningsdataene og det genererte bilde være.
...
generator_optimizer = tf.keras.optimizers.Adam(1e-4, beta_1=0.2)
discriminator_optimizer = tf.keras.optimizers.Adam(1e-4, beta_1=0.2)
#
